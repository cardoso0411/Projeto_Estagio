...
function database(): mysqli
{
    ...
    $port = (int) (getenv('DB_PORT') ?: 3300);

    $connection = new mysqli($host, $user, $password, $database, $port);

    if ($connection->connect_error) {
        throw new RuntimeException(sprintf('Nao foi possivel conectar ao banco de dados em %s:%d - %s', $host, $port, $connection->connect_error));
    }
